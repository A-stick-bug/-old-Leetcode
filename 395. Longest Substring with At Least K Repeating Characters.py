...
#     start, end = 0, 1
#     max = 0
#     chars = len(dict(Counter(s)))
#
#     for i in range(len(s) - 1):
#         sub = s[start:end]
#         unique = len(dict(Counter(sub)))
#
#         # print(min(dict(Counter(sub)).values()))
#         if min(dict(Counter(sub)).values()) < k:
#             end += 1
#         elif unique == chars:
#             if len(dict(Counter(sub))) > max:
#                 max = len(dict(Counter(sub)))
#             start += 1
#         else:
#             end += 1
#
